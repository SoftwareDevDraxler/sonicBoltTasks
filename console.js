...
const countriesTable = countries.map((country, index) => ({
    id: index + 1, 
    country: country
}));

console.table(countriesTable);
...
console.warn('Warning: This action may result in data loss!');
...
console.log(`While loop: ${end - start} ms`);
...
console.log(`For...of loop: ${end - start} ms`);

// Measure time for forEach loop
start = performance.now();
array.forEach(item => {
    item;
});
end = performance.now();
console.log(`forEach loop: ${end - start} ms`);
